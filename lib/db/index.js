...
Conn.prototype.dump = function(modelSchema, cb){
  var Model = modelType(modelSchema);
  	  Model.find({},cb);
  	  return this;
}

Conn.prototype.wipe = function(cb){
	// wipe collections
	var collNames = Object.keys(mongoose.connections[0].collections);
		collNames = collNames.map(function(modelName){
			return modelType(modelName);
		});
	async.parallel(collNames, function(Model){
...
Conn.prototype.import = function(modelSchema, models, cb){
	var Model = modelType(modelSchema);
	async.map(
			models,
...
exports.model = function(modelName, options){
	return crudify({
		'modelName':modelName,
		'options': options
	});
...
exports.crudify = crudify = function(Schema,realTime){
	// options---
	// realTime: Add event trigger support
	// lastMod : last modified field
	// created/updated 
	// listener: callback on create, remove
	// io: 
